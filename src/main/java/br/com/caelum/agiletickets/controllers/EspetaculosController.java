...
public class EspetaculosController {
	...
	public void adiciona(Espetaculo espetaculo) {
		...
		if (!validaEspetaculo(espetaculo)) {
	...
	public boolean validaEspetaculo(Espetaculo espetaculo) {
	...
	public void sessao(Long id) {
		...
		verificaSessaoNula(sessao);

		result.include("sessao", sessao);
	}

	public void verificaSessaoNula(Sessao sessao) {
		if (sessao == null) {
			result.notFound();
		}
	}
	...
	public void reserva(Long sessaoId, final Integer quantidade) {
		...
		if (!validaReserva(quantidade, sessao)){
	...
	public boolean validaReserva(final Integer quantidade, Sessao sessao) {
		if (quantidade < 1) {
			validator.add(new ValidationMessage(
		...
		if (!sessao.podeReservar(quantidade)) {
			validator.add(new ValidationMessage(
					"Nao existem ingressos disponíveis", ""));
			return false;
		}
		return true;
	}
	...
	private Espetaculo carregaEspetaculo(Long espetaculoId) {
		...
		if (espetaculo == null) {
			validator.add(new ValidationMessage("Não há espetáculos disponíveis", ""));
		}
